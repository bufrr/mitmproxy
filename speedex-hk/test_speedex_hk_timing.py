...
def turnkey_sign_response(now_ms=None):
    ...
    return json.dumps({
        "activity": {
            ...
            "result": {"signRawPayloadResult": {"r": "1a" * 32, "s": "2b" * 32, "v": "01"}},
            "votes": [], "fingerprint": "sha256:" + "00" * 8, "canApprove": False, "canReject": False,
...
def padre_sign_leg(run, anchor_hash=None, req_body="{}", manifest=None):
    ...
    f = FakeFlow("api.turnkey.com", "/public/v1/submit/sign_raw_payload", "POST", req_body,
...
class TestHkTiming(unittest.TestCase):
    ...
    def test_okx_full_leg(self):
        ...
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": "0x" + "ab" * 32}
        try:
            f = FakeFlow(
                "web3.okx.com",
                ...
                resp_body=json.dumps({"code": "0", "data": {"transactionHash": "0x" + "ab" * 32, "orderId": "ord1"}}),
            )
            A.addons[0].request(f)
            A.addons[0].response(f)
            time.sleep(0.05)
            # 创建帧不算成功
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "0", "orderId": "ord1"}}})))
            self.assertIsNone(A.STORE.timeline("run-okx")["legs"][0]["hkL1aMs"])
            # 成功帧
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "ord1", "transactionHash": "0x" + "ab" * 32}}})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
    ...
    def test_gmgn_processed_order_info(self):
        ...
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"value": [{"confirmationStatus": "confirmed", "err": None, "slot": 287654321}]}
        sig = "5" * 88
        try:
            f = FakeFlow("gmgn.ai", "/tapi/v1/swap_batch_order", "POST", resp_body=json.dumps({"data": {"hash": sig, "orderId": "g1"}}))
            A.addons[0].request(f)
            A.addons[0].response(f)
            A.addons[0].websocket_message(ws_frame("ws.gmgn.ai", json.dumps({"channel": "tg_processed_order_info", "data": [{"h": sig, "oi": "g1", "st": "successful", "si": "buy", "ch": "sol"}]})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
        ...
        self.assertEqual(leg["evidence"]["receipt"]["slot"], 287654321, "R24：Sol receipt 存 slot")
        # 卖单不采纳（新开窗口前先关旧窗口——单批纪律与生产一致）
        A.STORE.mark_close("run-g")
        A.STORE.mark_open("run-g2")
        f = FakeFlow("gmgn.ai", "/tapi/v1/swap_batch_order", "POST", resp_body=json.dumps({"data": {"hash": sig, "orderId": "g2"}}))
        A.addons[0].request(f)
        A.addons[0].response(f)
        A.addons[0].websocket_message(ws_frame("ws.gmgn.ai", json.dumps({"channel": "tg_processed_order_info", "data": [{"h": sig, "oi": "g2", "st": "successful", "si": "sell"}]})))
    ...
    def test_padre_msgpack_done(self):
        ...
        try:
            leg_int = padre_sign_leg("run-p", manifest={"version": 1, "runId": "run-p", "rounds": 1,
                                                        "legs": [{"platform": "padre", "chain": "bsc", "rounds": 1}]})
            self.assertEqual(leg_int["_anchor"], {}, "真实 Turnkey 响应不得建立任何锚（activity.id 不是订单号）")
            h = "0x" + "dd" * 32
            now_ms = int(time.time() * 1000)
            for node in ({"txnStatus": "DONE", "txnHash": h, "creationTime": now_ms},
                         {"txnStatus": "DONE", "txnHash": h, "orderId": TURNKEY_ACTIVITY_ID, "creationTime": now_ms},
                         {"txnStatus": "DONE", "txnHash": h, "orderId": "padre-order-77", "creationTime": now_ms}):
                A.addons[0].websocket_message(ws_frame("backend3.padre.gg", msgpack.packb([373, 200, node])))
            open_leg = A.STORE.timeline("run-p")["legs"][0]
            self.assertIsNone(open_leg["hkL1aMs"], "无独立锚 → DONE 不得算成功（含回显活动 UUID 的帧）")
        ...
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 0, "observed": 0, "unbound": 0})
        sign, ph = tl["legs"]
        self.assertEqual((sign["anchorRole"], sign["incomplete"]), ("sign", "no-execution-proof"))
        self.assertEqual(sign["evidence"]["anchor"]["keys"], [], "锚键为空——缺失原因在输出可见")
        self.assertTrue(sign["evidence"]["anchor"]["legacyRegexDiverged"], "旧正则会把 activity.id 当 orderId——分歧如实记录")
        self.assertEqual(ph["incomplete"], "no-order-observed", "授权槽占位保留")
        self.assertEqual(A.STORE.diag.get("wsFrameNoLegMatch"), 3, "三帧带身份却无腿可归属——计诊断，不静默")
        self.assertEqual(tl["diag"].get("wsFrameNoLegMatch"), 3, "per-run diag 同步透出")
    ...
    def test_padre_stale_replay_rejected(self):
        ...
        h_anchor = "0x" + "88" * 32
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": h_anchor}
        try:
            padre_sign_leg("run-ps", anchor_hash=h_anchor)
            old_ms = int((time.time() - 3600) * 1000)  # 1 小时前的重放帧
            # ①锚匹配但陈旧：纯新鲜性门拒收（不得凭锚匹配绕过陈旧门）
            stale = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h_anchor, "creationTime": old_ms}])
            ...
            mismatched = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": "0x" + "77" * 32, "creationTime": int(time.time() * 1000)}])
            ...
            self.assertEqual(leg1["txHash"], "0x88888888…888888", "冲突帧不得覆盖锚 hash")
            # ③锚匹配 + 新鲜（creationTime=now）正常采纳
            fresh = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h_anchor, "creationTime": int(time.time() * 1000)}])
    ...
    def test_binance_pending_then_finished(self):
        ...
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": "0x" + "ef" * 32}
        try:
            f = FakeFlow(
                "web3.binance.com",
                ...
                resp_body=json.dumps({"code": "000000", "success": True, "data": {"orderId": "oid-1", "clientOrderId": "cid-1"}}),
            )
            A.addons[0].request(f)
            A.addons[0].response(f)
            # PENDING：取 hash 不当成功
            A.addons[0].websocket_message(ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {"bizKey": "DEX_ALL_ORDER", "content": {"orderId": "oid-1", "clientOrderId": "cid-1", "status": "PENDING", "orderTxId": "0x" + "ef" * 32}}})))
            leg0 = A.STORE.timeline("run-b")["legs"][0]
            self.assertIsNone(leg0["hkL1aMs"], "PENDING 不是成功")
            self.assertEqual(leg0["txHash"], "0xefefefef…efefef", "PENDING 帧 hash 已收")
            # 别的订单的 FINISHED 不串单
            A.addons[0].websocket_message(ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {"bizKey": "DEX_ALL_ORDER", "content": {"orderId": "oid-OTHER", "status": "FINISHED"}}})))
            self.assertIsNone(A.STORE.timeline("run-b")["legs"][0]["hkL1aMs"], "orderId 不匹配不得采纳")
            A.addons[0].websocket_message(ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {"bizKey": "DEX_ALL_ORDER", "content": {"orderId": "oid-1", "clientOrderId": "cid-1", "status": "FINISHED", "orderTxId": "0x" + "ef" * 32}}})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
    ...
    def test_fomo_relay_three_way(self):
        ...
        rid = "0x" + "9a" * 32
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": p[0]}  # R24：echo 被轮询的 hash
        try:
            f = FakeFlow(
                "prod-api.fomo.family",
                "/swaps/v2",
                "POST",
                resp_body=json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}),
            )
            A.addons[0].request(f)
            A.addons[0].response(f)
            # 页面出站 subscribe（from_client）——绑定 requestId
            A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"type": "subscribe", "event": "request.status.updated", "filters": {"id": rid}}), from_client=True))
            # pending 帧不算
            A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"event": "request.status.updated", "data": {"status": "pending", "requestId": rid}})))
            self.assertIsNone(A.STORE.timeline("run-f")["legs"][0]["hkL1aMs"])
            A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"event": "request.status.updated", "data": {"status": "success", "requestId": rid, "txHashes": ["0x" + "12" * 32], "destinationChainId": 4663}})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
        leg = A.STORE.timeline("run-f")["legs"][0]
        self.assertEqual(leg["chain"], "robinhood", "destinationChainId 4663 → robinhood")
        self.assertIsNotNone(leg["hkL1aMs"])
    ...
    def test_rounds_occurrence_and_no_mark(self):
        ...
        self.assertEqual([x["observationIndex"] for x in legs], [1, 2, 3], "到达序连续编号")
        self.assertEqual([x["round"] for x in legs], [None, None, None], "r7：到达序不冒充授权轮")
    ...
    def test_stale_replay_no_pollution(self):
        ...
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": p[0]}
        h1 = "0x" + "aa" * 32
        h2 = "0x" + "bb" * 32
        ...
        def fin(oid, cid, h, **kw):
            ...
            if cid:
                c["clientOrderId"] = cid
            c.update(kw)
            return ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {"bizKey": "DEX_ALL_ORDER", "content": c}}))

        try:
            # r1 正常成交 oid-A/h1
            f1 = FakeFlow("web3.binance.com", po, "POST",
                          req_body=json.dumps({"clientOrderId": "c1", "chain": "BSC"}),
                          resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-A", "clientOrderId": "c1"}}))
            A.addons[0].request(f1)
            A.addons[0].response(f1)
            A.addons[0].websocket_message(fin("oid-A", "c1", h1))
            time.sleep(0.3)
            # r2 新腿 oid-B；重放 r1 的 FINISHED → 不得污染
            f2 = FakeFlow("web3.binance.com", po, "POST",
                          req_body=json.dumps({"clientOrderId": "c2", "chain": "BSC"}),
                          resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-B", "clientOrderId": "c2"}}))
        ...
        self.assertNotEqual(legs[0]["txHash"], legs[1]["txHash"], "同 hash 不得挂两条腿")
    ...
    def test_ws_messages_trimmed(self):
        ...
        f.websocket = types.SimpleNamespace(messages=[FakeWSMsg("{}") for _ in range(50)])
        for _ in range(3):
        ...
        f2.websocket.messages = [FakeWSMsg("{}") for _ in range(20)] + f2.websocket.messages
    ...
    def test_leg_fields_are_milliseconds(self):
        ...
        try:
            f = FakeFlow(
                ...
                req_body=json.dumps({"data": {"chainId": "56"}}),
                resp_body=json.dumps({"code": "0", "data": {"transactionHash": "0x" + "cd" * 32, "orderId": "o1"}}),
            )
            A.addons[0].request(f)
            A.addons[0].response(f)
            time.sleep(0.25)
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "o1", "transactionHash": "0x" + "cd" * 32}}})))
            time.sleep(0.2)
        finally:
            A._jsonrpc = orig
        leg = A.STORE.timeline("run-ms")["legs"][0]
        self.assertGreaterEqual(leg["hkL1aMs"], 200, "0.25s 睡眠后 hkL1aMs 应 ≥200ms（秒 bug 会给 0.2）")
        self.assertLess(leg["hkL1aMs"], 5000)
        for k in ("tOrderOutMs", "tFirstRespMs", "tSuccessPushMs"):
            self.assertGreater(leg[k], 10**6, f"{k} 应是毫秒级绝对值（秒域 uptime 需 >11 天才到 1e6）")
...
class TestR07PositiveCorrelation(unittest.TestCase):
    ...
    def test_fomo_success_requires_anchor(self):
        ...
        try:
            ...
            f2 = FakeFlow("prod-api.fomo.family", "/swaps/v2", "POST",
                          resp_body=json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}))
            A.addons[0].request(f2)
            A.addons[0].response(f2)
            A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"event": "request.status.updated", "data": {"status": "success", "txHashes": ["0x" + "34" * 32]}})))
            leg2 = A.STORE.timeline("run-fx")["legs"][1]
            self.assertIsNone(leg2["hkL1aMs"], "帧无 requestId → 不得凭时间窗充数")
            # 正向控制：帧 requestId ≡ 锚 → 采纳
            A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"event": "request.status.updated", "data": {"status": "success", "requestId": rid, "txHashes": ["0x" + "34" * 32], "destinationChainId": 4663}})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
        leg2 = A.STORE.timeline("run-fx")["legs"][1]
    ...
    def test_binance_frame_cannot_self_certify(self):
        ...
        try:
            ...
            f = FakeFlow("web3.binance.com", "/bapi/defi/v2/private/wallet-direct/web-dex/place-order", "POST",
                         req_body=json.dumps({"chain": "BSC"}), resp_body="{}")
            A.addons[0].request(f)
            A.addons[0].response(f)
            A.addons[0].websocket_message(ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {"bizKey": "DEX_ALL_ORDER", "content": {"orderId": "oid-X", "status": "FINISHED", "orderTxId": "0x" + "ef" * 32}}})))
            leg = A.STORE.timeline("run-sc")["legs"][0]
            self.assertIsNone(leg["hkL1aMs"], "帧自带 orderId 先写入腿再自证 → 不得算成功")
    ...
    def test_padre_done_needs_same_node_hash_and_fresh_ts(self):
        ...
        try:
            padre_sign_leg("run-pd", anchor_hash=h_anchor)  # r8：锚注入（生产无 Padre 锚源）
            # ①DONE 无 hash（有时间戳）→ 整帧拒收，不算成功
            A.addons[0].websocket_message(ws_frame("backend3.padre.gg", msgpack.packb([373, 200, {"txnStatus": "DONE", "creationTime": now_ms}])))
            ...
            stitched = msgpack.packb([373, 200, {"timestamp": now_ms, "sub": {"txnStatus": "DONE", "txnHash": h_anchor, "creationTime": old_ms}}])
            ...
            A.addons[0].websocket_message(ws_frame("backend3.padre.gg", msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h_anchor}])))
            leg = A.STORE.timeline("run-pd")["legs"][0]
            self.assertIsNone(leg["hkL1aMs"], "DONE 节点缺时间戳 → 不算成功")
            # 正向控制：同节点锚匹配 hash + 新鲜 creationTime → 采纳
            good = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h_anchor, "creationTime": now_ms}])
...
class TestR08ReceiptPoller(unittest.TestCase):
    ...
    def test_single_receipt_poller_latch(self):
        ...
        try:
            ...
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "o1", "transactionHash": h1}}})))
            time.sleep(0.05)
            # 冲突 hash 帧 → 整帧拒收
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "o1", "transactionHash": h2}}})))
            time.sleep(0.05)
        finally:
            A._poll_receipt = orig
...
class TestR09WindowIsolation(unittest.TestCase):
    ...
    def test_window_close_freezes_legs(self):
        ...
        try:
            ...
            f1 = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                          resp_body=json.dumps({"code": "0", "data": {"transactionHash": h1, "orderId": "oA"}}))
            A.addons[0].request(f1)
            A.addons[0].response(f1)
            A.STORE.mark_close("run-A")
            A.STORE.mark_open("run-B")
            # 属于 A 腿的迟到成功帧
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "oA", "transactionHash": h1}}})))
            leg_a = A.STORE.timeline("run-A")["legs"][0]
            self.assertIsNone(leg_a["hkL1aMs"], "关窗后帧不得更新 A 的腿")
            self.assertEqual(A.STORE.timeline("run-B")["legs"], [], "B 不得收到 A 腿的腿记录")
            self.assertEqual(list(A.STORE.events.get("run-B", [])), [], "A 腿的事件不得记进 B")
        finally:
            A._jsonrpc = orig
    ...
    def test_mark_open_closes_other_windows(self):
        ...
        self.assertIsNotNone(A.STORE.marks["run-old"]["closed"], "新窗开启须显式关闭旧窗")
...
def _http_json(method, url, obj=None):
    ...
    url = urllib.parse.urlunsplit(parsed._replace(netloc=f"127.0.0.1:{server.server_address[1]}"))
...
class TestR18R29ControlPlane(unittest.TestCase):
    ...
    def test_ctrl_takeover_and_split_reporting(self):
        ...
        A._start_ctrl()
        code, body = _http_json("GET", "http://127.0.0.1:8072/health")
        self.assertEqual((code, body["ok"], body["instanceId"]), (200, True, A.INSTANCE_ID))
...
class TestCtrlSendDisconnect(unittest.TestCase):
    ...
    def test_other_errors_propagate(self):
        ...
        for err in (
            OSError(errno.EAGAIN, "buffer full"),  # 非断连 OSError（BlockingIOError）不吞
            ValueError("boom"),
            RuntimeError("boom"),
        ):
            h = _bare_ctrl(_FakeWfile(error=err))
...
class _FakeWsConn:
    ...
    def recv(self, timeout=None):
        ...
        v = msgpack.unpackb(self.sent if isinstance(self.sent, bytes) else bytes(self.sent), raw=False)
...
class TestR21FrozenTimestamps(unittest.TestCase):
    ...
    def test_request_ts_frozen_at_entry(self):
        ...
        self.assertLess(leg["tOrderOutMs"], t_before + 400, "解析 0.6s 之后采样 = 入口未冻结")
        self.assertEqual(leg["chain"], "bsc")
    ...
    def test_ws_success_ts_frozen_at_entry(self):
        ...
        slow = lambda p: (time.sleep(0.6), [{"ids": {"orderId": "o1", "txHash": "0x" + "ab" * 32}, "success": True}])[1]
        ...
        try:
            ...
            A.addons[0].response(f)
            t_before = A._mono_ms()
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "o1", "transactionHash": "0x" + "ab" * 32}}})))
        finally:
            A._jsonrpc = orig_rpc
            A.RULES["okx"]["ws_entries"] = orig_entries
        leg = A.STORE.timeline("run-fw")["legs"][0]
        self.assertIsNotNone(leg["tSuccessPushMs"])
        self.assertLess(leg["tSuccessPushMs"], t_before + 400, "多次 decode 后才采样 = 入口未冻结")
...
class TestR22Manifest(unittest.TestCase):
    ...
    def test_mark_manifest_counts_and_placeholders(self):
        ...
        code, _ = _http_json("POST", "http://127.0.0.1:8072/mark", {"runId": "run-mf", "manifest": {"rounds": 3}})
        self.assertEqual(code, 200)
        f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST", "{}", "{}")
        A.addons[0].request(f)
        A.addons[0].response(f)
        ...
        self.assertEqual(code, 200)
        self.assertEqual(tl["counts"], {"requested": 3, "attempted": 1, "observed": 0, "unbound": 0})
    ...
    def test_manifest_detail_matching_and_backward_compat(self):
        ...
        A.STORE.mark_open("run-md", manifest={"legs": [{"platform": "okx", "chain": "bsc"}, {"platform": "okx", "chain": "bsc"}]})
        f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                     req_body=json.dumps({"data": {"chainId": "56"}}))
        A.addons[0].request(f)
        tl = A.STORE.timeline("run-md")
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 1, "observed": 0, "unbound": 0})
        ...
        A.addons[0].request(FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST", "{}"))
    ...
    def test_manifest_per_leg_rounds_denominator(self):
        ...
        A.STORE.mark_open("run-mr", manifest={"version": 1, "runId": "run-mr", "rounds": 3, "legs": legs_spec})
        # 实际只观察到 okx/bsc × 2 个下单请求
        for _ in range(2):
            A.addons[0].request(FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                                         req_body=json.dumps({"data": {"chainId": "56"}})))
        tl = A.STORE.timeline("run-mr")
        self.assertEqual(tl["counts"], {"requested": 36, "attempted": 2, "observed": 0, "unbound": 0},
                         "requested=Σ per-leg rounds（12 腿×3 轮）")
        self.assertEqual(len(tl["legs"]), 36, "2 真实腿 + 34 占位腿")
        ph = tl["legs"][2:]
        self.assertTrue(all(x["incomplete"] == "no-order-observed" and x["hkL1aMs"] is None for x in ph))
    ...
    def test_manifest_per_leg_rounds_preview_not_consumed(self):
        ...
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": p[0]}
        try:
            A.STORE.mark_open("run-mp", manifest={"version": 1, "runId": "run-mp", "rounds": 2,
                                                  "legs": [{"platform": "fomo", "chain": "robinhood", "rounds": 2}]})
            f = FakeFlow("prod-api.fomo.family", "/swaps/v2", "POST",
                         resp_body=json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}))
            A.addons[0].request(f)
            A.addons[0].response(f)
            tl = A.STORE.timeline("run-mp")
            self.assertEqual(tl["counts"], {"requested": 2, "attempted": 0, "observed": 0, "unbound": 0},
                             "预览腿不计 attempted")
            self.assertEqual(len(tl["legs"]), 3, "1 预览腿 + 2 占位（预览不消费授权轮次）")
            A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"event": "request.status.updated", "data": {"status": "success", "requestId": rid, "txHashes": ["0x" + "12" * 32], "destinationChainId": 4663}})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
        ...
        self.assertEqual(len(tl["legs"]), 2, "晋升后消费 1 个占位（1 order 腿 + 1 占位）")
    ...
    def test_chain_not_deduped_across_legs(self):
        ...
        try:
            for i, oid in enumerate(("o1", "o2")):
                ...
                f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                             resp_body=json.dumps({"code": "0", "data": {"transactionHash": h, "orderId": oid}}))
                A.addons[0].request(f)
                A.addons[0].response(f)
                A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": oid, "transactionHash": h, "chainId": "56"}}})))
            time.sleep(0.25)
        finally:
            A._jsonrpc = orig
...
class TestR24ReceiptIdentity(unittest.TestCase):
    ...
    def test_evm_receipt_transaction_hash_checked(self):
        ...
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "transactionHash": "0x" + "00" * 32}
        try:
            f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                         resp_body=json.dumps({"code": "0", "data": {"transactionHash": "0x" + "ab" * 32, "orderId": "o1"}}))
            A.addons[0].request(f)
...
class TestR25BoundedStore(unittest.TestCase):
    ...
    def test_bounded_retention_with_gone_marker(self):
        ...
        try:
            ...
            self.assertTrue(A.STORE.timeline("run-rr0").get("gone"), "最老 run 须标 gone")
            self.assertTrue(A.STORE.timeline("run-rr2").get("gone"))
            self.assertIsNone(A.STORE.timeline("run-rr6").get("gone"))
            self.assertEqual(len(A.STORE.timeline("run-rr6")["legs"]), 1, "最近 run 数据完整")
    ...
    def test_retention_pins_active_poller(self):
        ...
        try:
            ...
            self.assertIsNone(A.STORE.timeline("run-pin").get("gone"), "活动 poller 腿须 pin 住")
            leg["receiptPolling"] = False  # poller 结束
            self._mk_run("run-q3")
            self.assertTrue(A.STORE.timeline("run-pin").get("gone"), "poller 结束后可回收")
        finally:
            A.RETENTION_RUNS = orig_n
...
class TestR07SameEntryPadre(unittest.TestCase):
    ...
    def test_padre_hash_from_done_node_not_sibling(self):
        ...
        try:
            ...
            frame = msgpack.packb([373, 200, {
                "done": {"txnStatus": "DONE", "txnHash": hb, "creationTime": int(time.time() * 1000)},
                "other": {"txnHash": ha},
            }])
        ...
        self.assertIsNotNone(leg["hkL1aMs"], "DONE 节点同节点锚匹配 hash+新鲜时间戳 → 成功")
        self.assertEqual(leg["txHash"], "0xcdcdcdcd…cdcdcd", "hash 必须取自 DONE 节点本身，不得跨节点拼接")
    ...
    def test_padre_no_done_node_no_ids(self):
        ...
        frame = msgpack.packb([373, 200, {"txnStatus": "FILLED", "txnHash": "0x" + "55" * 32, "orderId": "po-1", "creationTime": int(time.time() * 1000)}])
        self.assertEqual(A._padre_ws_ids(frame), {}, "非 DONE 帧不提供任何 id/hash（含同层 orderId/hash）")
        A.addons[0].websocket_message(ws_frame("backend3.padre.gg", frame))
    ...
    def test_padre_done_without_anchor_rejected(self):
        ...
        try:
            ...
            frame = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": "0x" + "ab" * 32, "creationTime": int(time.time() * 1000)}])
            A.addons[0].websocket_message(ws_frame("backend3.padre.gg", frame))
            time.sleep(0.1)
        finally:
            A._poll_receipt = orig
    ...
    def test_padre_done_anchor_mismatch_rejected(self):
        ...
        A._poll_receipt = lambda *a, **k: polled.append(a)
        try:
            leg_int = padre_sign_leg("run-p7d", anchor_hash=h1)  # r8：锚注入（生产无 Padre 锚源）
            # 锚注入不经 response() 的闩锁——按 R08 由 HTTP 入口同款闩锁起 poller（轮询锚 hash）
            ph = A.STORE.claim_receipt_poll(leg_int)
            if ph:
                A._poll_receipt("run-p7d", leg_int, leg_int.get("chain"), ph, A._jsonrpc)
            frame = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h2, "creationTime": int(time.time() * 1000)}])
            A.addons[0].websocket_message(ws_frame("backend3.padre.gg", frame))
            time.sleep(0.1)
        finally:
            A._poll_receipt = orig
...
class TestR07BinanceHashConsistency(unittest.TestCase):
    ...
    def test_same_entry_hash_mismatch_rejects_whole_frame(self):
        ...
        try:
            f = FakeFlow(
                "web3.binance.com", self.PO, "POST",
                req_body=json.dumps({"clientOrderId": "c7", "chain": "BSC"}),
                resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-7", "clientOrderId": "c7"}}),
            )
            A.addons[0].request(f)
            A.addons[0].response(f)
            A.addons[0].websocket_message(self._fin("oid-7", "c7", orderTxId="0x" + "aa" * 32, txHash="0x" + "bb" * 32))
            time.sleep(0.3)
            leg = A.STORE.timeline("run-b7")["legs"][0]
            self.assertIsNone(leg["txHash"], "归一不一致帧的 hash 不得挂载（bug：先选 orderTxId=A）")
            ...
            A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1", "blockHash": "0x" + "99" * 32, "transactionHash": p[0]}
            A.addons[0].websocket_message(self._fin("oid-7", "c7", orderTxId="0x" + "cc" * 32, txHash="0x" + "cC" * 16 + "cc" * 16))
            time.sleep(0.3)
        finally:
            A._jsonrpc = orig
        leg = A.STORE.timeline("run-b7")["legs"][0]
...
class TestR09AtomicLegCreation(unittest.TestCase):
    ...
    def test_leg_creation_rejected_after_close(self):
        ...
        try:
            f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST", req_body=json.dumps({"data": {"chainId": "56"}}))
            A.addons[0].request(f)
        finally:
    ...
    def test_inflight_request_rejected_when_window_replaced(self):
        ...
        try:
            ...
            A.RULES["okx"]["extract_chain"] = orig
        self.assertEqual(A.STORE.timeline("run-r9a")["legs"], [], "旧窗（已关）不得接新腿")
        self.assertEqual(A.STORE.timeline("run-r9b")["legs"], [], "失效请求不得借新窗建腿")
        self.assertEqual(A.STORE.diag.get("orderReqNoWindow"), 1, "拒绝须计诊断计数")
        # 新窗开后到达的请求正常入窗
        A.addons[0].request(FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST", req_body=json.dumps({"data": {"chainId": "56"}})))
        self.assertEqual(len(A.STORE.timeline("run-r9b")["legs"]), 1, "新窗只接纳其后到达的请求")
    ...
    def test_same_runid_reopen_invalidates_inflight_request(self):
        ...
        try:
            ...
            A.RULES["okx"]["extract_chain"] = orig
        self.assertEqual(A.STORE.timeline("run-r9c")["legs"], [], "旧 epoch 捕获的请求不得借重开窗建腿")
        self.assertEqual(A.STORE.diag.get("orderReqNoWindow"), 1, "epoch 失效拒绝须计诊断计数")
        A.addons[0].request(FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST", req_body=json.dumps({"data": {"chainId": "56"}})))
        self.assertEqual(len(A.STORE.timeline("run-r9c")["legs"]), 1, "重开后到达的请求正常入窗")
...
class TestR18StoreHandover(unittest.TestCase):
    ...
    def test_takeover_marks_dropped_runs(self):
        ...
        reg = A._ctrl_registry()
        reg["knownRuns"] = {"instanceId": "old-instance-x", "runs": ["run-ho1"], "at": "t"}
        fresh()
        A._start_ctrl()  # 接管：停旧控制面 + rebind + 捕获 droppedRuns
        try:
            tl = A.STORE.timeline("run-ho1")
            self.assertTrue(tl.get("gone"), "旧实例 run 应标 gone")
            self.assertTrue(tl.get("droppedInReload"), "须区分 dropped（重载丢失）与从未见过/retention 回收")
            self.assertIsNone(A.STORE.timeline("run-never").get("droppedInReload"), "从未见过的 run 不标 dropped")
            code, body = _http_json("GET", "http://127.0.0.1:8072/health")
            self.assertEqual(code, 200)
            tk = body.get("takeover")
            self.assertTrue(tk and tk.get("droppedRuns") >= 1, "/health 须暴露「接管自旧实例，N 个 run 被丢弃」")
...
class TestR20GmgnWarmProbe(unittest.TestCase):
    def test_https_warm_keepalive_second_request_not_halved(self):
        ...
        r = A._probe_https_warm("gmgn.ai", "/defi/quotation/v1/chains", conn_cls=factory)
        self.assertIsNotNone(r)
        self.assertEqual(r["method"], "https-warm")
        self.assertEqual(FakeConn.instances, 1, "预热+热样本须同一连接")
        self.assertEqual(holder["conn"].requests, 2, "1 预热 + 1 热样本（EU httpsWarmRtt 同口径）")
        self.assertEqual(r["ms"], r["rttMs"], "https-warm 整 RTT 不减半")
        self.assertGreaterEqual(r["rttMs"], 80, "单次 0.1s → 热样本 ~100ms；减半/冷样本口径给不出")
...
class TestR21R29TimelineIdentity(unittest.TestCase):
    ...
    def test_timeline_method_and_identity(self):
        ...
        A.get_egress = lambda: (_ for _ in ()).throw(AssertionError("timeline 不得为 vantage 发网"))
        try:
            ...
            self.assertEqual(tl["pythonVersion"], _pf.python_version(), "与 /health build manifest 同源")
            self.assertEqual(tl["mitmproxyVersion"], A._mitmproxy_version(), "键必须存在（离线环境值可为 null）")
            self.assertEqual(tl["vantage"], "unknown-proxy", "无实测 egress → unknown-proxy")
            A._EGRESS_CACHE["data"] = {"countryCode": "SG", "label": "Singapore"}
            A._EGRESS_CACHE["at"] = time.monotonic()
            self.assertEqual(A.STORE.timeline("run-id1")["vantage"], "sg-proxy", "实测 countryCode → 小写-proxy")
            # gone 响应同样带身份字段
    ...
    def test_latency_vantage_derived_from_egress(self):
        ...
        orig_get = A.get_egress
        saved = {}
        for fn in ("_probe_https_warm", "_probe_okx_ws", "_probe_padre_ws", "_probe_http_ping", "_probe_fomo_ws"):
    ...
    def test_egress_country_code_cached(self):
        ...
        class R:
            ...
            def read(self):
                return json.dumps({"status": "success", "query": "203.0.113.7", "country": "Hong Kong", "countryCode": "HK", "regionName": "HK", "city": "HK", "isp": "doc-test-net"}).encode()
...
class TestR22PreviewRoleAndBudgets(unittest.TestCase):
    ...
    def test_fomo_preview_leg_not_attempted_until_executed(self):
        ...
        try:
            ...
            for _ in range(2):
                f = FakeFlow(
                    "prod-api.fomo.family", "/swaps/v2", "POST",
                    resp_body=json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}),
                )
                A.addons[0].request(f)
                A.addons[0].response(f)
            tl = A.STORE.timeline("run-fp")
            self.assertEqual(tl["counts"]["attempted"], 0, "预览不计 attempted（bug：每个预览 POST 都当下单 attempt）")
            previews = [x for x in tl["legs"] if x["platform"] == "fomo"]
            self.assertTrue(previews and all(x.get("anchorRole") == "preview" for x in previews), "fomo 腿须标 anchorRole=preview")
            self.assertEqual(len(tl["legs"]), 3, "2 预览腿 + 1 manifest 占位（预览不消费授权轮次）")
    ...
    def test_window_budgets_legs_and_events(self):
        ...
        try:
            ...
            self.assertEqual((tl.get("overflow") or {}).get("legs"), 2, "超出腿数如实标 overflow")
            A.MAX_LEGS_PER_RUN, A.MAX_EVENTS_PER_RUN = 100, 2
            A.STORE.mark_open("run-bud2")
            for _ in range(4):
                A.addons[0].request(FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST", "{}"))
            tl2 = A.STORE.timeline("run-bud2")
            self.assertEqual((tl2.get("overflow") or {}).get("events"), 2, "超出事件数如实标 overflow")
        finally:
...
class TestR22SlotBinding(unittest.TestCase):
    ...
    def test_duplicate_same_order_request_merges_into_slot(self):
        ...
        po = "/bapi/defi/v2/private/wallet-direct/web-dex/place-order"
        A.STORE.mark_open("run-dup", manifest={"version": 1, "runId": "run-dup", "rounds": 1,
                                               "legs": [{"platform": "binance", "chain": "bsc", "rounds": 1}]})
        for _ in range(2):  # 同订单两次请求（产品重试/重复派发）
            f = FakeFlow("web3.binance.com", po, "POST",
                         req_body=json.dumps({"clientOrderId": "cid-dup", "chain": "BSC"}),
                         resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-dup", "clientOrderId": "cid-dup"}}))
            A.addons[0].request(f)
            A.addons[0].response(f)
        tl = A.STORE.timeline("run-dup")
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 1, "observed": 0, "unbound": 0},
        ...
        self.assertEqual(bound[0]["round"], None, "r7：绑定只证明槽容量——授权轮号不由槽位序推导（null=未知）")
        ...
        self.assertEqual(len([x for x in tl["legs"] if x["incomplete"] == "no-order-observed"]), 0,
                         "槽已被首请求消费——不再有缺失占位")
        self.assertEqual(A.STORE.diag.get("orderReqDuplicate", 0), 1, "同单重复须留诊断计数")
        self.assertEqual(A.STORE.diag.get("orderReqUnbound", 0), 0, "正向同单不再误记 unbound 诊断")
    ...
    def test_unknown_chain_request_cannot_claim_chain_slot(self):
        ...
        po = "/bapi/defi/v2/private/wallet-direct/web-dex/place-order"
        A.STORE.mark_open("run-uc", manifest={"version": 1, "runId": "run-uc", "rounds": 1,
                                              "legs": [{"platform": "binance", "chain": "bsc", "rounds": 1},
                                                       {"platform": "binance", "chain": "solana", "rounds": 1}]})
        f = FakeFlow("web3.binance.com", po, "POST",
                     req_body=json.dumps({"clientOrderId": "cid-x"}),  # 无 chain 字段
                     resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-x", "clientOrderId": "cid-x"}}))
        A.addons[0].request(f)
        A.addons[0].response(f)
        self.assertTrue(A.STORE.legs["run-uc"][0]["pendingBind"])
        A.STORE.mark_close("run-uc")
        tl = A.STORE.timeline("run-uc")
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 0, "observed": 0, "unbound": 1},
    ...
    def test_padre_sign_role_not_attempted_until_done(self):
        ...
        h = "0x" + "cd" * 32
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: None  # 本例不验 receipt——成功推送即 observed
        try:
            # r8：锚注入（生产今日无 Padre 锚源——真实 Turnkey 响应无身份，见 test_padre_msgpack_done）
            padre_sign_leg("run-sign", anchor_hash=h, req_body=json.dumps({"data": {"chainId": "56"}}),
                           manifest={"version": 1, "runId": "run-sign", "rounds": 1,
                                     "legs": [{"platform": "padre", "chain": "bsc", "rounds": 1}]})
            tl0 = A.STORE.timeline("run-sign")
            self.assertEqual(tl0["legs"][0]["anchorRole"], "sign", "签名调用先标 sign（未证明一一对应 order）")
            self.assertEqual(tl0["counts"], {"requested": 1, "attempted": 0, "observed": 0, "unbound": 0},
                             "sign 腿不计 attempted")
            self.assertEqual(len(tl0["legs"]), 2, "sign 腿不消费槽位——缺失占位保留")
            frame = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h, "creationTime": int(time.time() * 1000)}])
            A.addons[0].websocket_message(ws_frame("backend3.padre.gg", frame))
        finally:
            A._jsonrpc = orig
        tl = A.STORE.timeline("run-sign")
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 1, "observed": 1, "unbound": 0},
                         "DONE 执行证据落地后计 1 次下单")
        leg = [x for x in tl["legs"] if x["anchorRole"] == "order"][0]
        self.assertEqual(leg["round"], None, "r7：晋升绑槽不写授权轮号（到达序≠授权轮；唯一关联由 EU 侧回配 authorizedRound）")
    ...
    def test_padre_sign_without_done_gets_no_execution_proof(self):
        ...
        padre_sign_leg("run-sx", manifest={"version": 1, "runId": "run-sx", "rounds": 1,
                                           "legs": [{"platform": "padre", "chain": "bsc", "rounds": 1}]})
        ...
        self.assertEqual(sign_leg["incomplete"], "no-execution-proof",
                         "无执行证据的 sign 腿关窗后须如实标注（旧 bug：incomplete=null）")
        self.assertEqual(tl["legs"][1]["incomplete"], "no-order-observed", "授权槽占位仍在")
    ...
    def test_window_close_backfills_missing_states(self):
        ...
        A.STORE.mark_open("run-ms", manifest={"version": 1, "runId": "run-ms", "rounds": 2,
        ...
        try:
            # 腿1：无任何结果
            A.addons[0].request(FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                                         req_body=json.dumps({"data": {"chainId": "56"}}), resp_body="{}"))
            # 腿2：有成功无 receipt
            f2 = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                          req_body=json.dumps({"data": {"chainId": "56"}}),
                          resp_body=json.dumps({"code": "0", "data": {"transactionHash": h1, "orderId": "o2"}}))
            A.addons[0].request(f2)
            A.addons[0].response(f2)
            A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"}, "data": {"dexData": {"status": "1", "orderId": "o2", "transactionHash": h1}}})))
            open_tl = A.STORE.timeline("run-ms")
            self.assertIsNone(open_tl["legs"][0]["incomplete"], "开窗中仍在采集——不补缺失状态")
            A.STORE.mark_close("run-ms")
        finally:
            A._jsonrpc = orig
        tl = A.STORE.timeline("run-ms")
        self.assertEqual(tl["legs"][0]["incomplete"], "no-success-observed", "关窗后无结果腿须补 no-success")
        self.assertEqual(tl["legs"][1]["incomplete"], "no-receipt-observed", "有成功无 receipt 须补 no-receipt")
        self.assertIsNotNone(tl["legs"][1]["hkL1aMs"], "成功观察不受影响")
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 2, "observed": 1, "unbound": 0})
    ...
    def test_unpromoted_preview_close_marked_no_execution_proof(self):
        ...
        A.STORE.mark_open("run-pv", manifest={"version": 1, "runId": "run-pv", "rounds": 1,
                                              "legs": [{"platform": "fomo", "chain": "robinhood", "rounds": 1}]})
        A.addons[0].request(FakeFlow("prod-api.fomo.family", "/swaps/v2", "POST", "", "{}"))
        ...
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 0, "observed": 0, "unbound": 0})
        self.assertEqual(tl["legs"][1]["incomplete"], "no-order-observed", "预览不占槽——占位保留")
    ...
    def test_freeze_backfill_is_terminal(self):
        ...
        self.assertEqual(tl1["legs"][0]["incomplete"], "receipt poll timeout", "已有 incomplete 不被补写覆盖")
        self.assertEqual(tl1["legs"][0]["incomplete"], tl2["legs"][0]["incomplete"], "重复拉取补写一致")
        ...
        self.assertEqual(A.STORE.timeline("run-tm2")["legs"][0]["incomplete"], "no-success-observed")
...
class TestR24BlockIdentityGate(unittest.TestCase):
    ...
    def test_evm_receipt_missing_block_identity_incomplete(self):
        ...
        A.RECEIPT_POLL_INTERVAL_S = 0.1
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "transactionHash": p[0]}  # 无块身份
        try:
            f = FakeFlow(
                "web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                resp_body=json.dumps({"code": "0", "data": {"transactionHash": "0x" + "ab" * 32, "orderId": "o1"}}),
    ...
    def test_sol_status_missing_slot_incomplete(self):
        ...
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"value": [{"confirmationStatus": "confirmed", "err": None}]}  # 无 slot
        sig = "5" * 88
        try:
            f = FakeFlow("gmgn.ai", "/tapi/v1/swap_batch_order", "POST", resp_body=json.dumps({"data": {"hash": sig, "orderId": "g1"}}))
            A.addons[0].request(f)
...
class TestP3Cleanup(unittest.TestCase):
    ...
    def test_padre_done_node_strict_only(self):
        ...
        self.assertIsNone(A._padre_done_node({"orderStatus": "filled"}), "宽松词表反例：filled 不得当 DONE")
...
class TestP3PadreFreshDoneFixture(unittest.TestCase):
    ...
    def test_first_user_click_ms_fresh_accept_stale_reject(self):
        ...
        A.addons[0].websocket_message(self._frame({"txnStatus": "DONE", "txnHash": h, "firstUserClickMs": now_ms - 6.7 * 3600_000}))
        self.assertIsNone(A.STORE.timeline("run-f1")["legs"][0]["hkL1aMs"], "陈旧 firstUserClickMs 重放不得算成功")
        A.STORE.mark_close("run-f1")
        # 锚正相关 + 新鲜 firstUserClickMs → 采纳
        self._open_leg("run-f2", h)
        A.addons[0].websocket_message(self._frame({"txnStatus": "DONE", "txnHash": h, "firstUserClickMs": now_ms}))
    ...
    def test_no_time_fallback_without_anchor(self):
        ...
        now_ms = int(time.time() * 1000)
        A.addons[0].websocket_message(self._frame({"txnStatus": "DONE", "txnHash": "0x" + "cd" * 32, "firstUserClickMs": now_ms}))
    ...
    def test_time_fields_scoped_to_done_subtree(self):
        ...
        A.addons[0].websocket_message(self._frame({"txnStatus": "DONE", "txnHash": h, "exec": {"creationTime": now_ms}}))
        ...
        A.addons[0].websocket_message(self._frame({"timestamp": now_ms, "sub": {"txnStatus": "DONE", "txnHash": h}}))
        leg = A.STORE.timeline("run-f5")["legs"][0]
        self.assertIsNone(leg["hkL1aMs"], "DONE 子树外（根/父）的新鲜 timestamp 不得背书")
    ...
    def test_creation_time_seconds_epoch_converted(self):
        ...
        A.addons[0].websocket_message(self._frame({"txnStatus": "DONE", "txnHash": h, "creationTime": int(time.time())}))
        leg = A.STORE.timeline("run-f6")["legs"][0]
        self.assertIsNotNone(leg["hkL1aMs"], "秒级 epoch creationTime ×1000 后判新鲜 → 采纳")
...
class TestR22OrderIdentityDedup(unittest.TestCase):
    ...
    def _binance(self, cid=None, oid=None, chain="BSC"):
        ...
        f = FakeFlow("web3.binance.com", self.PO, "POST",
    ...
    def test_n2_same_order_repeat_merges(self):
        ...
        A.STORE.mark_open("run-d2", manifest={"version": 1, "runId": "run-d2", "rounds": 2,
    ...
    def test_n2_two_distinct_orders_take_own_slots(self):
        ...
        A.STORE.mark_open("run-dd", manifest={"version": 1, "runId": "run-dd", "rounds": 2,
                                              "legs": [{"platform": "binance", "chain": "bsc", "rounds": 2}]})
        self._binance("cid-A", "oid-A")
        self._binance("cid-B", "oid-B")
        tl = A.STORE.timeline("run-dd")
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 2, "observed": 0, "unbound": 0})
        self.assertEqual([x["round"] for x in tl["legs"]], [None, None],
                         "r7：各占其槽（容量绑定），授权轮号不由到达序推导")
        self.assertEqual([x["observationIndex"] for x in tl["legs"]], [1, 2], "到达序连续")
    ...
    def test_n2_repeat_then_distinct_binds_released_slot(self):
        ...
        A.STORE.mark_open("run-dm", manifest={"version": 1, "runId": "run-dm", "rounds": 2,
                                              "legs": [{"platform": "okx", "chain": "bsc", "rounds": 2}]})
        ...
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 2, "observed": 0, "unbound": 0})
        self.assertEqual([x["round"] for x in tl["legs"]], [None, None, None],
    ...
    def test_identity_from_response_converts_unbound_to_dup(self):
        ...
        A.STORE.mark_open("run-dc", manifest={"version": 1, "runId": "run-dc", "rounds": 1,
        ...
        self.assertEqual(A.STORE.diag.get("orderReqUnbound", 0), 0, "改判 dup 后不留假 unbound 诊断")
        self.assertEqual(A.STORE.diag.get("orderReqDuplicate"), 1)
    ...
    def test_repeat_without_identity_stays_unbound(self):
        ...
        A.STORE.mark_open("run-dn", manifest={"version": 1, "runId": "run-dn", "rounds": 1,
                                              "legs": [{"platform": "binance", "chain": "bsc", "rounds": 1}]})
        self._binance()  # 无 clientOrderId，响应无 id
        self._binance()
        tl = A.STORE.timeline("run-dn")
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 1, "observed": 0, "unbound": 1})
    ...
    def test_conflicting_identity_not_deduped(self):
        ...
        A.STORE.mark_open("run-dx", manifest={"version": 1, "runId": "run-dx", "rounds": 1,
                                              "legs": [{"platform": "binance", "chain": "bsc", "rounds": 1}]})
        ...
        self.assertEqual(internal[1].get("orderId"), "oid-001", "响应身份已建立——确为冲突而非缺失")
    ...
    def test_fomo_duplicate_execution_not_rebound(self):
        ...
        rid = "0x" + "9a" * 32
        A.STORE.mark_open("run-fd", manifest={"version": 1, "runId": "run-fd", "rounds": 1,
                                              "legs": [{"platform": "fomo", "chain": "robinhood", "rounds": 1}]})
...
class TestR22DoneReceiptOutOfOrder(unittest.TestCase):
    ...
    RH_RECEIPT = {"rpc": "https://rpc.mainnet.chain.robinhood.com", "chain": "robinhood",
                  "pollIntervalMs": 800, "status": 1, "blockHash": "0x" + "99" * 32, "blockNumber": "0x1"}

    def setUp(self):
        fresh()
        orig = A._poll_receipt
        A._poll_receipt = lambda *a, **k: None  # receipt 由测试手动 settle——事件顺序确定性
        self.addCleanup(setattr, A, "_poll_receipt", orig)

    def _padre_sign_leg(self, run, manifest_chain, h):
        """chain 未知的 sign 腿（body "{}"）+ 注入独立 hash 锚（r8：生产无 Padre 锚源）。"""
        return padre_sign_leg(run, anchor_hash=h,
                              manifest={"version": 1, "runId": run, "rounds": 1,
    ...
    def _done(h):
        ...
        return ws_frame("backend3.padre.gg", msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h, "creationTime": int(time.time() * 1000)}]))

    def _assert_final(self, tl):
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 1, "observed": 1, "unbound": 0})
        self.assertEqual(len(tl["legs"]), 1, "授权槽被补绑消费——无占位")
        leg = tl["legs"][0]
        self.assertEqual((leg["chain"], leg["anchorRole"], leg["round"], leg["unbound"]), ("robinhood", "order", None, False))
    ...
    def test_okx_unknown_chain_waits_for_receipt_before_manifest_binding(self):
        ...
        A.STORE.mark_open(run, manifest={"version": 1, "runId": run, "rounds": 1,
            "legs": [{"platform": "okx", "chain": "solana", "rounds": 1}]})
        with A.STORE.lock:
            leg = A.STORE._new_leg_locked(run, "okx", None, {"t": time.monotonic() * 1000, "tw": "synthetic"})
        ...
        with A.STORE.lock:
            extra = A.STORE._new_leg_locked(run, "okx", None, {"t": time.monotonic() * 1000, "tw": "synthetic"})
    ...
    def test_done_then_receipt_and_reverse_same_outcome(self):
        ...
        self.assertTrue(leg1.get("pendingBind"), "链未实证——等 receipt 实证链补绑（旧 bug：直接 unbound）")
        mid = A.STORE.timeline("run-o1")
        self.assertEqual(mid["counts"], {"requested": 1, "attempted": 0, "observed": 0, "unbound": 0},
        ...
        self.assertEqual(leg2.get("chain"), "robinhood", "receipt 实证链立即回填（旧 bug：leg.chain 恒 null）")
        mid2 = A.STORE.timeline("run-o2")
        self.assertEqual(mid2["counts"]["observed"], 0, "sign 腿 receipt 不单独计 observed（③门槛）")
    ...
    def test_done_without_receipt_settles_unbound_at_close(self):
        ...
        self.assertEqual(A.STORE.diag.get("orderReqUnbound", 0), 0, "开窗中不提前判 unbound")
        A.STORE.mark_close("run-o3")
        tl = A.STORE.timeline("run-o3")
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 0, "observed": 0, "unbound": 1},
        ...
        self.assertEqual(A.STORE.diag.get("orderReqUnbound"), 1)
        self.assertEqual(tl["legs"][1]["incomplete"], "no-order-observed", "授权槽未被消费——占位保留")
    ...
    def test_receipt_chain_without_manifest_slot_not_bound(self):
        ...
        self.assertEqual(tl["legs"][1]["incomplete"], "no-order-observed", "bsc 授权槽不被实证 robinhood 的腿消费")
...
class TestR22ObservedBoundOrderGate(unittest.TestCase):
    ...
    def test_sign_leg_receipt_alone_not_observed(self):
        ...
        h = "0x" + "ab" * 32
        leg = padre_sign_leg("run-ob", anchor_hash=h,
                             manifest={"version": 1, "runId": "run-ob", "rounds": 1,
        ...
        self.assertEqual(tl["legs"][1]["incomplete"], "no-order-observed", "授权槽占位保留")

    def test_preview_leg_receipt_alone_not_observed(self):
        """fomo preview 腿同律：只有 receipt 没有 Relay success → observed=0。"""
        rid = "0x" + "9a" * 32
        A.STORE.mark_open("run-op", manifest={"version": 1, "runId": "run-op", "rounds": 1,
                                              "legs": [{"platform": "fomo", "chain": "robinhood", "rounds": 1}]})
        f = FakeFlow("prod-api.fomo.family", "/swaps/v2", "POST",
                     resp_body=json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}))
        A.addons[0].request(f)
    ...
    def test_bound_order_leg_receipt_counts_observed(self):
        ...
        h = "0x" + "ab" * 32
        A.STORE.mark_open("run-oo", manifest={"version": 1, "runId": "run-oo", "rounds": 1,
                                              "legs": [{"platform": "okx", "chain": "bsc", "rounds": 1}]})
        f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                     req_body=json.dumps({"data": {"chainId": "56"}}),
                     resp_body=json.dumps({"code": "0", "data": {"transactionHash": h, "orderId": "o1"}}))
        A.addons[0].request(f)
        A.addons[0].response(f)
        leg = A.STORE.legs["run-oo"][0]
        A._settle_receipt(leg, {"rpc": "https://bsc-dataseed1.binance.org", "chain": "bsc",
                                "pollIntervalMs": 800, "status": 1, "blockHash": "0x" + "99" * 32, "blockNumber": "0x1"})
...
class TestR7ObservationVsAuthorizedRound(unittest.TestCase):
    ...
    def _binance(self, cid, chain="Robinhood"):
        f = FakeFlow("web3.binance.com", self.PO, "POST",
                     ...
                     resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-" + cid, "clientOrderId": cid}}))
        A.addons[0].request(f)
        A.addons[0].response(f)
    ...
    def test_first_round_missing_no_round_impersonation(self):
        ...
        A.STORE.mark_open("run-r7", manifest={"version": 1, "runId": "run-r7", "rounds": 5,
                                              "legs": [{"platform": "binance", "chain": "robinhood", "rounds": 5}]})
        for cid in ("cid-r2", "cid-r3", "cid-r4", "cid-r5"):  # 授权 r1 未广播
            self._binance(cid)
        tl = A.STORE.timeline("run-r7")
        real = [x for x in tl["legs"] if x["anchorRole"] is not None]
        self.assertEqual([x["round"] for x in real], [None] * 4, "授权轮未知——不按到达序冒充")
        self.assertEqual([x["observationIndex"] for x in real], [1, 2, 3, 4], "到达序连续编号（纯观察事实）")
        self.assertEqual(tl["counts"], {"requested": 5, "attempted": 4, "observed": 0, "unbound": 0},
                         "槽绑定/计数口径不变")
        ph = [x for x in tl["legs"] if x["incomplete"] == "no-order-observed"]
    ...
    def test_observation_index_covers_dup_and_unbound(self):
        ...
        A.STORE.mark_open("run-oi", manifest={"version": 1, "runId": "run-oi", "rounds": 1,
                                              "legs": [{"platform": "binance", "chain": "bsc", "rounds": 1}]})
        for cid in ("cid-a", "cid-a", "cid-b"):  # 第二次同单 → dup；第三次容量已满 → unbound
    ...
    def test_padre_done_new_hosts_classified(self):
        ...
        self.assertIsNone(A._classify_ws("nottxn-service.padre.gg"), "点边界——同后缀异主机不误分类")
        # 端到端：新 host 的 DONE 帧走同一份锚+新鲜度门 → 晋升 order 成功
        # （r8：锚注入——生产今日无 Padre 锚源，见 test_padre_msgpack_done）
        h = "0x" + "cd" * 32
        padre_sign_leg("run-b2", anchor_hash=h, req_body=json.dumps({"data": {"chainId": "56"}}),
                       manifest={"version": 1, "runId": "run-b2", "rounds": 1,
                                 "legs": [{"platform": "padre", "chain": "bsc", "rounds": 1}]})
        frame = msgpack.packb([373, 200, {"txnStatus": "DONE", "txnHash": h, "creationTime": int(time.time() * 1000)}])
        A.addons[0].websocket_message(ws_frame("txn-service.padre.gg", frame))
        leg = A.STORE.timeline("run-b2")["legs"][0]
        self.assertEqual(leg["anchorRole"], "order", "新 host DONE 正向匹配 = 执行证据 → 晋升 order")
        self.assertIsNotNone(leg["hkL1aMs"])
...
class TestR8StructuredAnchors(unittest.TestCase):
    ...
    def test_binance_resp_ids_separate_keys_key_order_numeric_nested_id(self):
        ...
        body_b = '{"code":"000000","success":true,"data":{"clientOrderId":"%s","orderId":"oid-B-0001","message":"Waiting for Signing"}}' % cid
        self.assertEqual(A._binance_resp_ids(body_b), {"orderId": "oid-B-0001", "clientOrderId": cid})
        body_c = '{"code":"000000","success":true,"data":{"orderId":1234567890123456789,"clientOrderId":"%s"}}' % cid
        self.assertEqual(A._binance_resp_ids(body_c), {"orderId": "1234567890123456789", "clientOrderId": cid},
                         "数字 orderId 归一为字串（旧正则要求引号 → 漏配 → clientOrderId 值冒充 orderId）")
        body_e = '{"code":"000000","success":true,"data":{"wallet":{"id":"wallet-000001"},"orderId":"oid-E-0001","clientOrderId":"%s"}}' % cid
        self.assertEqual(A._binance_resp_ids(body_e)["orderId"], "oid-E-0001", "嵌套 \"id\" 不是订单号")
        self.assertEqual(A._binance_resp_ids('{"code":"100001005","success":false,"message":"x"}'), {}, "失败 ack 无身份")
        self.assertEqual(A._binance_resp_ids('{"code":"000000","data":{"orderId":true}}'), {}, "bool 不是身份")
    ...
    def test_binance_resp_ids_same_node_hash_normalized_or_ambiguous(self):
        ...
        h = "0x" + "ab" * 32
        out = A._binance_resp_ids(json.dumps({"code": "000000", "data": {"orderId": "o1", "orderTxId": h, "txHash": h.upper().replace("0X", "0x")}}))
        self.assertEqual(out["txHash"], h, "大小写变体同值 → 归一后取 hash")
        amb = A._binance_resp_ids(json.dumps({"code": "000000", "data": {"orderId": "o1", "orderTxId": h, "txHash": "0x" + "cd" * 32}}))
        self.assertNotIn("txHash", amb)
        self.assertEqual(amb["_ambiguous"], ["txHash"])
        sig = "5" * 88
        self.assertEqual(A._binance_resp_ids(json.dumps({"code": "000000", "data": {"orderId": "o2", "signature": sig}}))["txHash"], sig)
    ...
    def test_okx_gmgn_resp_ids_whitelist_and_ambiguity(self):
        ...
        h = "0x" + "ab" * 32
        okx = A._okx_resp_ids(json.dumps({"code": "0", "data": {"id": "row-000001", "clientOrderId": "not-okx", "orderId": "ord-1", "transactionHash": h, "isSuccess": True}}))
        self.assertEqual(okx, {"orderId": "ord-1", "txHash": h})
        okx_amb = A._okx_resp_ids(json.dumps({"data": [{"orderId": "ord-1"}, {"orderId": "ord-2"}]}))
        self.assertNotIn("orderId", okx_amb)
        self.assertEqual(okx_amb["_ambiguous"], ["orderId"])
        self.assertEqual(A._okx_resp_ids(json.dumps({"data": {"id": "row-000001"}})), {}, "id 不是订单号")
        sig = "5" * 88
        gm = A._gmgn_resp_ids(json.dumps({"code": 0, "data": {"hash": sig, "orderId": "g1"}}))
        self.assertEqual(gm, {"orderId": "g1", "txHash": sig})
        self.assertEqual(A._gmgn_resp_ids(json.dumps({"data": {"hash": "0xshort", "oi": "g2"}})), {"orderId": "g2"}, "非法 hash 形状不取")
    ...
    def test_padre_turnkey_response_has_no_anchor_legacy_regex_diverged(self):
        ...
        self.assertEqual(A._legacy_regex_ids_diag(body), {"orderId": TURNKEY_ACTIVITY_ID}, "旧正则误锚：activity.id → orderId")

    def test_fomo_resp_ids_structured_path(self):
        rid = "0x" + "9a" * 32
        out = A._fomo_resp_ids(json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}))
        self.assertEqual(out, {"orderId": rid, "chain": "robinhood"})
        self.assertEqual(A._fomo_resp_ids(json.dumps({"responseObject": {"v2Swap": {"relaySwapId": "nope"}}})), {}, "非 EVM 形状不取")
        self.assertEqual(A._fomo_resp_ids(json.dumps({"statusCode": 200, "message": "Successful v2 swap"})), {}, "同链 DFlow 预览无 v2Swap → 无锚")
...
class TestR8IdentityRouting(unittest.TestCase):
    ...
    def _manifest(self, run, rounds=1, platform="binance", chain="bsc"):
        return {"version": 1, "runId": run, "rounds": rounds, "legs": [{"platform": platform, "chain": chain, "rounds": rounds}]}

    def _place(self, cid, resp_body, chain="BSC"):
        f = FakeFlow("web3.binance.com", self.PO, "POST", req_body=json.dumps({"clientOrderId": cid, "chain": chain}), resp_body=resp_body)
        A.addons[0].request(f)
        A.addons[0].response(f)
        return f
    ...
    def test_key_order_finished_attributed(self):
        ...
        self._place(cid, '{"code":"000000","success":true,"data":{"clientOrderId":"%s","orderId":"oid-B-0001"}}' % cid)
        A.addons[0].websocket_message(self._fin("oid-B-0001", cid, "0x" + "b1" * 32))
        tl = A.STORE.timeline(run, full=True)
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 1, "observed": 1, "unbound": 0})
        leg = tl["legs"][0]
        self.assertIsNotNone(leg["hkL1aMs"])
        self.assertEqual((leg["orderId"], leg["clientOrderId"]), ("oid-B-0001", cid), "分键锚")
        self.assertEqual(leg["evidence"]["anchor"]["keys"], ["clientOrderId", "orderId"], "WS 学得的 hash 不是锚（腿字段有、锚键无）")
        self.assertEqual(leg["txHash"], "0x" + "b1" * 32, "正向匹配帧的 hash 挂载为腿字段")
        self.assertTrue(leg["evidence"]["anchor"]["legacyRegexDiverged"], "旧正则在此 body 上会误锚——分歧如实记录")
        self.assertEqual(leg["evidence"]["successAnchorKeys"], ["orderId", "clientOrderId"])
    ...
    def test_numeric_order_id_finished_attributed(self):
        ...
        self._place(cid, '{"code":"000000","success":true,"data":{"orderId":1234567890123456789,"clientOrderId":"%s"}}' % cid)
        # 帧 orderId 为数字
        A.addons[0].websocket_message(self._fin(1234567890123456789, cid, "0x" + "c1" * 32))
        tl = A.STORE.timeline(run, full=True)
        self.assertEqual(tl["counts"]["observed"], 1)
        ...
        self._place(cid + "-2", '{"code":"000000","success":true,"data":{"orderId":987654321,"clientOrderId":"%s-2"}}' % cid)
        A.addons[0].websocket_message(self._fin("987654321", cid + "-2", "0x" + "c2" * 32))
    ...
    def test_platform_resend_success_lands_on_formal_leg(self):
        ...
        f2 = self._place(cid, '{"code":"000000","success":true,"data":{"orderId":"oid-A-0001","clientOrderId":"%s"}}' % cid)
        internal = A.STORE.legs[run]
        self.assertEqual((internal[0]["slot"], internal[0]["dup"]), (0, False), "首请求 = formal（绑槽）")
        self.assertTrue(internal[1]["dup"])
        self.assertIs(internal[1]["_dupOf"], internal[0])
        self.assertEqual(internal[0]["_anchor"].get("orderId"), "oid-A-0001", "dup 腿 ack 的 orderId 并入 formal 锚（独立锚，非 WS 学得）")
        ...
        self.assertEqual(formal["tOrderOutMs"], internal[0]["tOrderOutMs"], "起点 = 首次下单请求")
        self.assertEqual(formal["txHash"], "0x" + "a1" * 32, "hash 挂 formal")
        self.assertEqual(formal["incomplete"], "no-receipt-observed")
        self.assertEqual(formal["evidence"]["successAnchorKeys"], ["orderId", "clientOrderId"])
        self.assertTrue(dup["dup"])
        self.assertIsNone(dup["hkL1aMs"], "成功永不留在 dup 腿")
        self.assertEqual(dup["incomplete"], "duplicate-of-bound-leg", "dup 腿关窗如实标注（非 no-success-observed）")
        self.assertEqual(dup["evidence"]["mergedIntoFlowId"], formal["evidence"]["orderFlowId"])
    ...
    def test_nested_id_key_not_order_anchor(self):
        ...
        self._place(cid, '{"code":"000000","success":true,"data":{"wallet":{"id":"wallet-000001"},"orderId":"oid-E-0001","clientOrderId":"%s"}}' % cid)
    ...
    def test_dup_with_conflicting_order_id_not_attributed(self):
        ...
        self._place(cid, '{"code":"000000","success":true,"data":{"orderId":"oid-F-0001","clientOrderId":"%s"}}' % cid)
        self._place(cid, '{"code":"000000","success":true,"data":{"orderId":"oid-F-0002","clientOrderId":"%s"}}' % cid)
        internal = A.STORE.legs[run]
        self.assertFalse(internal[1].get("dup"), "r9：冲突身份推翻 dup 判定——重开为独立候选")
        self.assertIsNone(internal[1].get("_dupOf"))
        self.assertTrue(internal[1].get("unbound"), "无空槽——重开后按容量判定落 unbound（计数如实）")
        self.assertTrue(internal[1].get("_multiOrderSuspect"), "multi-order-suspect 内部审计标记")
        self.assertEqual(A.STORE.diag.get("orderReqDuplicate", 0), 0, "重开后 dup 计数同步改记")
        ...
        A.STORE.mark_close(run)
        tl = A.STORE.timeline(run, full=True)
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 1, "observed": 1, "unbound": 1},
                         "formal 计 attempted/observed；重开腿 unbound 诊断不计")
        self.assertIsNotNone(tl["legs"][1]["hkL1aMs"], "第二订单的执行证据落重开腿（旧码：整帧吞掉）")
        self.assertIsNotNone(tl["legs"][0]["hkL1aMs"])
        self.assertEqual(tl["legs"][0]["orderId"], "oid-F-0001", "formal 锚不被 dup 的冲突 orderId 覆盖")
        self.assertIn("orderId", tl["legs"][1]["evidence"]["anchor"]["conflicts"])
        self.assertGreaterEqual(tl["diag"].get("anchorConflict", 0), 1)
        self.assertEqual(tl["diag"].get("orderDupReopened"), 1, "重开如实可见（multi-order-suspect）")
    ...
    def test_late_success_for_previous_round_routed_by_identity(self):
        ...
        self._place("cid-g1", '{"code":"000000","success":true,"data":{"orderId":"oid-G-0001","clientOrderId":"cid-g1"}}')
        self._place("cid-g2", '{"code":"000000","success":true,"data":{"orderId":"oid-G-0002","clientOrderId":"cid-g2"}}')
        A.addons[0].websocket_message(self._fin("oid-G-0001", "cid-g1", "0x" + "91" * 32))  # r1 迟到成功
        mid = A.STORE.timeline(run)
        self.assertIsNotNone(mid["legs"][0]["hkL1aMs"], "迟到成功帧按身份落 r1 腿（旧 bug：整帧丢弃）")
        self.assertIsNone(mid["legs"][1]["hkL1aMs"])
        A.addons[0].websocket_message(self._fin("oid-G-0002", "cid-g2", "0x" + "92" * 32))
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 2, "observed": 2, "unbound": 0})
    ...
    def test_ws_frame_without_leg_match_is_counted_not_silent(self):
        ...
        run = "run-r8n"
        A.STORE.mark_open(run, manifest=self._manifest(run))
        self._place("cid-n1", '{"code":"000000","success":true,"data":{"orderId":"oid-N-0001","clientOrderId":"cid-n1"}}')
        A.addons[0].websocket_message(self._fin("oid-OTHER", "cid-other", "0x" + "77" * 32))  # 他单：全不等
        A.addons[0].websocket_message(ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {"bizKey": "ticker", "content": {"p": "1.0"}}})))
        tl = A.STORE.timeline(run)
        ...
        A.addons[0].websocket_message(self._fin("oid-OTHER", "cid-n1", "0x" + "78" * 32))  # 一等一不等：冲突
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["diag"], {"wsFrameNoLegMatch": 1, "wsFrameAnchorConflict": 1})
    ...
    def test_no_manifest_resend_earliest_order_leg_wins(self):
        ...
        self._place(cid, '{"code":"000000","success":true,"data":{"orderId":"oid-NM-1","clientOrderId":"%s"}}' % cid)
    ...
    def test_fomo_same_relay_id_prefers_latest_preview(self):
        ...
        rid = "0x" + "9a" * 32
        A.STORE.mark_open("run-r8fo", manifest=self._manifest("run-r8fo", platform="fomo", chain="robinhood"))
        for _ in range(2):
            f = FakeFlow("prod-api.fomo.family", "/swaps/v2", "POST",
                         resp_body=json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": rid, "destinationChainId": 4663}}}))
            A.addons[0].request(f)
        ...
        self.assertEqual(A.STORE.timeline("run-r8fo")["counts"], {"requested": 1, "attempted": 1, "observed": 1, "unbound": 0})
    ...
    def test_timeline_schema_additions_backward_compatible(self):
        ...
        run = "run-r8s"
        A.STORE.mark_open(run, manifest=self._manifest(run, rounds=2))
        self._place("cid-s1", '{"code":"000000","success":true,"data":{"orderId":"oid-S-0001","clientOrderId":"cid-s1"}}')
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["schemaVersion"], 2)
        self.assertIsInstance(tl["diag"], dict)
        leg, ph = tl["legs"]
        for k in ("chain", "platform", "round", "observationIndex", "anchorRole", "unbound", "dup", "pendingBind",
                  "tOrderOutMs", "tFirstRespMs", "tSuccessPushMs", "tReceiptMs", "hkL1aMs", "hkL3Ms", "txHash",
                  "orderId", "clientOrderId", "evidence", "incomplete", "atWall"):
            self.assertIn(k, leg)
        self.assertEqual(set(leg["evidence"]), {"orderFlowId", "wsFrameSha", "receipt", "anchor", "successAnchorKeys", "mergedIntoFlowId"})
        self.assertEqual(set(leg["evidence"]["anchor"]), {"keys", "legacyRegexDiverged", "conflicts", "ambiguous"})
        self.assertIsNone(leg["orderId"], "缺省模式不暴露身份值")
        self.assertNotIn("oid-S-0001", json.dumps(leg["evidence"]), "evidence 不含身份值")
...
class TestR9AnchorConflictVeto(unittest.TestCase):
    ...
    def _manifest(self, run, rounds=2):
        return {"version": 1, "runId": run, "rounds": rounds,
                "legs": [{"platform": "fomo", "chain": "robinhood", "rounds": rounds}]}

    def _preview(self, relay=None, respond=True):
        body = "{}"
        if relay:
            body = json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": relay, "destinationChainId": 4663}}})
    ...
    def _respond(self, f, relay):
        f.response = FakeResp(json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": relay, "destinationChainId": 4663}}}))
        A.addons[0].response(f)

    def _subscribe(self, rid):
        A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps(
            {"type": "subscribe", "event": "request.status.updated", "filters": {"id": rid}}), from_client=True))

    def _status(self, rid, status, txh=None):
        d = {"status": status, "requestId": rid, "destinationChainId": 4663}
        if txh:
            d["txHashes"] = [txh]
        A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps({"event": "request.status.updated", "data": d})))
    ...
    def test_interleaved_ab_cross_bound_subscribe_vetoed(self):
        ...
        self.assertTrue(lb.get("_anchorVeto"), "B 锚自相矛盾 → 否决（旧码：只记 conflict 不动）")
        self.assertEqual(lb.get("incomplete"), "anchor-conflict", "否决即时落盘（首写）")
        self.assertEqual((lb.get("_anchor") or {}).get("orderId"), self.RELAY_A, "锚首写冻结——冲突值不覆盖")
        ...
        self.assertIsNone(lb.get("tSuccessPushMs"), "冲突帧不得驱动 B 成功（旧码：落在 B）")
        self.assertEqual(lb.get("anchorRole"), "preview", "被否决腿不晋升")
        self.assertIsNone(lb.get("slot"))
        self._status(self.RELAY_B, "success", "0x" + "34" * 32)   # B 的成功帧——B 锚冻结 relayA，正向不可达
        self.assertIsNone(lb.get("tSuccessPushMs"))
        A.STORE.mark_close(run)
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 1, "observed": 1, "unbound": 0},
                         "只有 A 计 attempted/observed——observed 不因冲突帧增加")
        out_a, out_b = tl["legs"][0], tl["legs"][1]
        self.assertLess(la["tOrderOutMs"], lb["tOrderOutMs"])
        self.assertEqual(out_a["hkL1aMs"], round(la["tSuccessPushMs"] - la["tOrderOutMs"], 1),
        ...
        self.assertEqual(tl["diag"].get("wsFrameNoLegMatch"), 1, "relayB 成功帧无腿可归属——如实计数不静默")
        self.assertEqual([p[3] for p in self.polls], ["0x" + "12" * 32],
    ...
    def test_cancel_repeat_success_ordering(self):
        ...
        self.assertEqual(p2.get("anchorRole"), "order", "最晚一致 preview 晋升（口径不变）")
    ...
    def test_vetoed_leg_only_match_counted_visible(self):
        ...
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 1, "attempted": 0, "observed": 0, "unbound": 0})
        self.assertEqual(tl["diag"].get("wsFrameAnchorVeto"), 1, "per-run diag 同步透出")
...
class TestR9OrderIdentityBoundaries(unittest.TestCase):
    ...
    @staticmethod
    def _receipt_ok():
        return {"rpc": "https://rpc.invalid", "chain": "bsc", "pollIntervalMs": 800,
                "status": 1, "blockHash": "0x" + "11" * 32, "blockNumber": 7}

    @staticmethod
    def _fin(oid, cid, h):
        return ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {
            "bizKey": "DEX_ALL_ORDER",
            "content": {"orderId": oid, "clientOrderId": cid, "status": "FINISHED", "orderTxId": h}}}))
    ...
    def test_response_identity_earliest_request_becomes_formal(self):
        ...
        run = "run-r9e"
        A.STORE.mark_open(run, manifest={"version": 1, "runId": run, "rounds": 2,
                                         "legs": [{"platform": "binance", "chain": "bsc", "rounds": 2}]})
        fx = self._req()
        time.sleep(0.02)
        fy = self._req()
        ...
        self.assertEqual(tl["legs"][0]["hkL1aMs"], round(lx["tSuccessPushMs"] - lx["tOrderOutMs"], 1),
                         "成功时刻按最早请求（平台重发含在 L1a′ 内，docs/binance.md §90 同口径）")
        self.assertEqual(tl["legs"][1]["incomplete"], "duplicate-of-bound-leg")
        self.assertEqual(tl["legs"][1]["evidence"]["mergedIntoFlowId"], fx.id,
    ...
    def test_transfer_moves_settled_success_to_earliest_leg(self):
        ...
        A.addons[0].websocket_message(self._fin("oid-T1", "cid-T1", "0x" + "71" * 32))  # 成功先落 Y
        ly = A.STORE.legs[run][1]
        self.assertIsNotNone(ly.get("tSuccessPushMs"))
        self._resp(fx, "oid-T1", "cid-T1")   # X 身份揭示 → 移交
        lx, ly = A.STORE.legs[run]
        self.assertIsNotNone(lx.get("tSuccessPushMs"), "已落地成功随 formal 资格移交最早请求")
        ...
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 1, "observed": 1, "unbound": 0})
        self.assertEqual(tl["legs"][0]["hkL1aMs"], round(lx["tSuccessPushMs"] - lx["tOrderOutMs"], 1))
        self.assertGreater(tl["legs"][0]["hkL1aMs"], 15.0, "起点前移到最早请求——时长含 X→Y 间隔")
    ...
    def test_cross_platform_same_order_id_not_merged(self):
        ...
        fo = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                      req_body=json.dumps({"data": {"chainId": "56"}}),
                      resp_body=json.dumps({"code": "0", "data": {"transactionHash": "0x" + "ab" * 32, "orderId": "SAME-1"}}))
        ...
        A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({
            ...
            "data": {"dexData": {"status": "1", "orderId": "SAME-1", "transactionHash": "0x" + "ab" * 32}}})))
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 2, "observed": 2, "unbound": 0},
    ...
    def test_dup_leg_conflicting_order_id_reopens_independent(self):
        ...
        self.assertTrue(lz.get("_multiOrderSuspect"), "multi-order-suspect 内部审计标记")
        self.assertEqual(lz.get("slot"), 0, "有空槽——按自身身份重新绑槽")
        self.assertEqual(A.STORE.diag.get("orderReqDuplicate", 0), 0, "重开改记 dup 计数")
        self.assertEqual(A.STORE.diag.get("orderDupReopened"), 1)
    ...
    def test_formal_veto_reopens_dup_children(self):
        ...
        self.assertTrue(lf.get("_anchorVeto"), "formal 锚（含 dup 并入值）与自身响应冲突 → 否决")
        self.assertEqual(lf.get("incomplete"), "anchor-conflict")
        self.assertEqual((lf.get("_anchor") or {}).get("orderId"), "oid-V2", "锚首写冻结——冲突值不覆盖")
        ...
        self.assertIsNone(lf.get("tSuccessPushMs"), "oid-V1 帧与两腿均冲突——不归属（不猜）")
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 2, "observed": 1, "unbound": 0},
                         "被否决 formal 占 attempted（槽已消费）但不计 observed")
        self.assertEqual(tl["diag"].get("wsFrameAnchorConflict"), 1, "oid-V1 帧拒收可见")
    ...
    def test_transfer_inflight_poller_receipt_redirects_to_new_formal(self):
        ...
        **移交**不是复制——Y 闩锁释放，否则关窗后 run 被 R25 pin 死永不回收。"""
        ...
        self.assertIsNotNone(lx.get("tReceiptMs"), "receipt 经 dup→formal 重定向落新 formal")
        self.assertIsNone(ly.get("tReceiptMs"), "receipt 同成功一样永不留在 dup 腿")
        A.STORE.mark_close(run)
        with A.STORE.lock:
            self.assertFalse(A.STORE._run_pinned_locked(run), "无遗留闩锁——run 可正常回收（R25）")
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 1, "observed": 1, "unbound": 0})
        self.assertEqual(tl["legs"][0]["hkL3Ms"], round(lx["tReceiptMs"] - lx["tOrderOutMs"], 1),
                         "L3′ 起点同样按最早请求")
    ...
    def test_transfer_inflight_poller_timeout_redirects_to_new_formal(self):
        ...
        self.assertEqual(lx.get("incomplete"), "receipt poll timeout", "失败回写同样重定向到新 formal")
        self.assertIsNone(ly.get("incomplete"))
        self.assertFalse(ly.get("receiptPolling"))
        A.STORE.mark_close(run)
        with A.STORE.lock:
            self.assertFalse(A.STORE._run_pinned_locked(run), "闩锁+incomplete/receipt 必居其一——不 pin 死")
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 1, "observed": 0, "unbound": 0})
    ...
    def test_veto_after_receipt_excluded_from_observed(self):
        ...
        self.assertEqual(tl["counts"]["observed"], 0, "receipt 先于否决落定 → counts 层排除（observed 不增加）")
        self.assertEqual(tl["legs"][0]["incomplete"], "anchor-conflict")
...
class TestR10LateVetoRevocation(unittest.TestCase):
    ...
    def test_success_then_conflict_revokes_qualified_metrics(self):
        ...
        A.STORE.mark_open(run, manifest={"version": 1, "runId": run, "rounds": 2,
                                         "legs": [{"platform": "fomo", "chain": "robinhood", "rounds": 2}]})
        fb = FakeFlow("prod-api.fomo.family", "/swaps/v2", "POST", req_body=json.dumps({"chainId": 4663}))
        A.addons[0].request(fb)                  # B 请求（响应稍后）
        A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps(
            {"type": "subscribe", "event": "request.status.updated", "filters": {"id": self.RELAY_A}}), from_client=True))
        A.addons[0].websocket_message(ws_frame("ws.relay.link", json.dumps(
            {"event": "request.status.updated", "data": {"status": "success", "requestId": self.RELAY_A,
                                                         "txHashes": ["0x" + "12" * 32], "destinationChainId": 4663}})))
        leg = A.STORE.legs[run][0]
        self.assertIsNotNone(leg.get("tSuccessPushMs"), "成功先于否决落定（内部原始时间保留——证据不删）")
        self.assertEqual(leg.get("anchorRole"), "order", "已被成功帧晋升（内部事实保留）")
        recorded = round(leg["tSuccessPushMs"] - leg["tOrderOutMs"], 1)
        fb.response = FakeResp(json.dumps({"success": True, "responseObject": {"v2Swap": {"relaySwapId": self.RELAY_B, "destinationChainId": 4663}}}))
        A.addons[0].response(fb)                 # 迟到响应明确 relayB → 冲突 → 否决
        self.assertTrue(leg.get("_anchorVeto"))
        A.STORE.mark_close(run)
        tl = A.STORE.timeline(run)
        out = tl["legs"][0]
        self.assertEqual(out["anchorVeto"], True, "腿带 anchorVeto:true——不再以 bound/eligible 呈现")
        self.assertIsNone(out["hkL1aMs"], "合格 L1a′ 撤销（r9 残余：仍导出 1000ms 入页面统计）")
        ...
        self.assertEqual(out["incomplete"], "anchor-conflict", "否决原因保留（首写落盘）")
        self.assertIsNotNone(out["tSuccessPushMs"], "原始事件时间仍在私有 raw（诊断用，不删证据）")
        self.assertIsNotNone(out["evidence"]["wsFrameSha"])
        self.assertEqual(tl["counts"], {"requested": 2, "attempted": 1, "observed": 0, "unbound": 0},
                         "槽已消费仍占 attempted；observed 不增加")
        ph = tl["legs"][1]
        self.assertEqual((ph["anchorVeto"], ph["hkL1aMsVetoed"], ph["hkL3MsVetoed"]), (False, None, None),
                         "占位腿形状齐整")
    ...
    def test_receipt_then_conflict_revokes_l3(self):
        ...
        f = FakeFlow("web3.binance.com", "/bapi/defi/v2/private/wallet-direct/web-dex/place-order", "POST",
                     ...
                     resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-R1", "clientOrderId": "cid-R1", "txHash": h}}))
        A.addons[0].request(f)
        A.addons[0].response(f)
        leg = A.STORE.legs[run][0]
        A._settle_receipt(leg, {"rpc": "https://rpc.invalid", "chain": "bsc", "pollIntervalMs": 800,
                                "status": 1, "blockHash": "0x" + "11" * 32, "blockNumber": 7})
        self.assertIsNotNone(leg.get("tReceiptMs"))
    ...
    def test_clean_leg_serialization_shape(self):
        ...
        f = FakeFlow("web3.binance.com", "/bapi/defi/v2/private/wallet-direct/web-dex/place-order", "POST",
                     ...
                     resp_body=json.dumps({"code": "000000", "data": {"orderId": "oid-OK", "clientOrderId": "cid-OK"}}))
        A.addons[0].request(f)
        A.addons[0].response(f)
        A.addons[0].websocket_message(ws_frame("web3-stream.binance.com", json.dumps({"stream": "w3pc:x", "data": {
            "bizKey": "DEX_ALL_ORDER", "content": {"orderId": "oid-OK", "clientOrderId": "cid-OK", "status": "FINISHED", "orderTxId": "0x" + "0a" * 32}}})))
...
class TestR10GmgnOkxPerEntry(unittest.TestCase):
    ...
    def _gmgn_order(self, run, resp_body):
        f = FakeFlow("gmgn.ai", "/tapi/v1/swap_batch_order", "POST",
                     req_body=json.dumps({"chain": "sol"}), resp_body=resp_body)
        A.addons[0].request(f)
        A.addons[0].response(f)
        return f
    ...
    def test_gmgn_cross_entry_no_manufactured_conflict(self):
        ...
        self.assertNotIn("orderId", ids, "跨 entry 不拼接身份（旧码：oi(ordA)+hash(hB) 拼成同一锚）")
        self.assertNotIn("txHash", ids)
        self.assertEqual(ids.get("_ambiguous"), ["orderId", "txHash"], "无法证明同单 → fail-closed + 歧义可见")
        run = "run-r10xa"
        A.STORE.mark_open(run, manifest={"version": 1, "runId": run, "rounds": 1,
                                         "legs": [{"platform": "gmgn", "chain": "solana", "rounds": 1}]})
        self._gmgn_order(run, body)
        leg = A.STORE.legs[run][0]
        self.assertEqual(leg.get("_anchor") or {}, {}, "无锚——成功帧结构上不可能正向关联")
        A.addons[0].websocket_message(ws_frame("ws.gmgn.ai", json.dumps({"channel": "tg_processed_order_info", "data": [
            {"h": "3" * 88, "oi": "ordA", "st": "successful", "si": "buy", "ch": "sol"}]})))
        self.assertFalse(leg.get("_anchorVeto", False), "不制造假冲突/误否决")
        self.assertIsNone(leg.get("tSuccessPushMs"))
        A.STORE.mark_close(run)
        tl = A.STORE.timeline(run)
        self.assertNotIn("anchorConflict", tl["diag"], "无跨 entry 拼接 → 无锚冲突计数（旧码 wsFrameAnchorConflict）")
        self.assertNotIn("wsFrameAnchorConflict", tl["diag"])
        self.assertEqual(tl["diag"].get("wsFrameNoLegMatch"), 1, "无锚腿的成功帧零命中——如实计数不吞")
        self.assertEqual(tl["legs"][0]["incomplete"], "no-success-observed")
        self.assertEqual(tl["legs"][0]["evidence"]["anchor"]["ambiguous"], ["orderId", "txHash"])
        self.assertEqual(tl["counts"]["observed"], 0)
    ...
    def test_gmgn_second_entry_identity_associated(self):
        ...
        A.STORE.mark_open(run, manifest={"version": 1, "runId": run, "rounds": 1,
                                         "legs": [{"platform": "gmgn", "chain": "solana", "rounds": 1}]})
        self._gmgn_order(run, json.dumps({"code": 0, "data": [{"note": "noise-entry"}, {"oi": "g1", "hash": self.SIG_A}]}))
        leg = A.STORE.legs[run][0]
        self.assertEqual((leg.get("_anchor") or {}).get("orderId"), "g1", "第二 entry 的身份照常建立锚")
        A.addons[0].websocket_message(ws_frame("ws.gmgn.ai", json.dumps({"channel": "tg_order_info", "data": [
            {"h": self.SIG_B, "oi": "gOther", "st": "successful", "si": "buy", "ch": "sol"},   # 他单成功（entry 0）
            {"h": self.SIG_A, "oi": "g1", "st": "successful", "si": "buy", "ch": "sol"}]})))   # 本单成功（entry 1）
        self.assertIsNotNone(leg.get("tSuccessPushMs"), "目标在第二 entry 照常采纳（旧码：data[0]-only 漏报）")
        self.assertEqual(leg.get("txHash"), self.SIG_A, "hash 取本单 entry——不取 entry 0 的他单 hash")
        tl = A.STORE.timeline(run)
        self.assertEqual(tl["counts"]["observed"], 1)
        self.assertEqual(tl["diag"].get("wsFrameNoLegMatch"), 1, "他单 entry 零命中如实计数")
        self.assertEqual(tl["legs"][0]["evidence"]["successAnchorKeys"], ["orderId", "txHash"])
    ...
    def test_okx_second_entry_associated(self):
        ...
        amb = A._okx_resp_ids(json.dumps({"code": "0", "data": [{"orderId": "ord-1"}, {"transactionHash": h1}]}))
        self.assertNotIn("orderId", amb)
        self.assertEqual(amb.get("_ambiguous"), ["orderId", "txHash"])
        # 单元级：正向一致的多个 entry 并集（同单证据分散在多 entry 仍可关联）
        same = A._okx_resp_ids(json.dumps({"code": "0", "data": [{"orderId": "ord-1", "transactionHash": h1}, {"orderId": "ord-1"}]}))
        self.assertEqual(same, {"orderId": "ord-1", "txHash": h1}, "共同键全等的 entry 可并集")
        run = "run-r10o2"
        A.STORE.mark_open(run, manifest={"version": 1, "runId": run, "rounds": 1,
                                         "legs": [{"platform": "okx", "chain": "bsc", "rounds": 1}]})
        f = FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/batchBroadcast", "POST",
                     req_body=json.dumps({"data": {"chainId": "56"}}),
                     resp_body=json.dumps({"code": "0", "data": [{"orderId": "ord1", "transactionHash": h1}]}))
        A.addons[0].request(f)
        A.addons[0].response(f)
        A.addons[0].websocket_message(ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-across-order-info"}, "data": [
            {"dexData": {"status": "1", "orderId": "ordOther", "transactionHash": h2, "chainId": "56"}},
            {"dexData": {"status": "1", "orderId": "ord1", "transactionHash": h1, "chainId": "56"}}]})))
        leg = A.STORE.legs[run][0]
...
class TestR10EarlyWsBuffer(unittest.TestCase):
    ...
    def setUp(self):
        fresh()
        orig = A._jsonrpc
        A._jsonrpc = lambda url, m, p, timeout=10: {"status": "0x1", "blockNumber": "0x1",
                                                    "blockHash": "0x" + "99" * 32, "transactionHash": p[0]}
        self.addCleanup(setattr, A, "_jsonrpc", orig)

    def _manifest(self, run):
        return {"version": 1, "runId": run, "rounds": 1, "legs": [{"platform": "okx", "chain": "bsc", "rounds": 1}]}

    def _okx_req(self, resp_oid="ord1", resp_h=H1):
        return FakeFlow("web3.okx.com", "/priapi/v6/dx/trade/multi/broadcast", "POST",
                        req_body=json.dumps({"data": {"chainId": "56"}}),
                        resp_body=json.dumps({"code": "0", "data": {"transactionHash": resp_h, "orderId": resp_oid}}))

    def _okx_success(self, oid="ord1", h=H1):
        return ws_frame("wsdexpri.okx.com", json.dumps({"arg": {"channel": "dex-swap-order-info"},
                                                        "data": {"dexData": {"status": "1", "orderId": oid, "transactionHash": h, "chainId": "56"}}}))
    ...
    def test_success_before_http_response_adopted_with_source_time(self):
        ...
        leg = A.STORE.legs[run][0]
        self.assertIsNone(leg.get("tSuccessPushMs"), "锚未建立——早到帧不得直接采纳（缓冲待定）")
        ...
        self.assertLess(leg["tSuccessPushMs"], leg["tFirstRespMs"], "成功时刻 = 帧源时间（先于响应）")
        time.sleep(0.25)
        tl = A.STORE.timeline(run)
        out = tl["legs"][0]
        self.assertEqual(out["hkL1aMs"], round(leg["tSuccessPushMs"] - leg["tOrderOutMs"], 1))
        self.assertGreaterEqual(out["hkL1aMs"], 40.0, "L1a′ 含请求→成功帧真实间隔（不从响应起算）")
    ...
    def test_early_frame_window_closed_not_adopted(self):
        ...
        self.assertEqual(A.STORE.diag.get("wsFrameNoLegMatch"), 1, "缓冲单元零命中终态——如实计数不静默")
    ...
    def test_early_frame_conflict_not_adopted(self):
        ...
        self.assertEqual(A.STORE.diag.get("wsFrameAnchorConflict"), 1, "冲突终态如实计数")
    ...
    def test_buffer_gate_requires_anchorless_leg(self):
        ...
        A.addons[0].websocket_message(self._okx_success(oid="ord-OTHER", h="0x" + "55" * 32))
        self.assertNotIn("wsFrameBuffered", A.STORE.diag, "无锚待定腿 → 不缓冲")
        self.assertEqual(A.STORE.diag.get("wsFrameNoLegMatch"), 1, "零命中即终态计数（不推迟）")
...
class TestR10CtrlPortEnv(unittest.TestCase):
    ...
    def test_port_resolver_invalid_falls_back(self):
        for bad in ("", "   ", "abc", "18072.5", "0x46A8", "-1", "0", "65536", "99999", "True", "1e4"):
            os.environ["SPEEDEX_HK_CTRL_PORT"] = bad
            self.assertEqual(A._ctrl_port(), 8072, f"非法值 {bad!r} → 回退 8072（不崩溃）")
    ...
    def test_listener_follows_env_port(self):
        ...
        self.assertEqual((code, body["ok"]), (200, True))
        self.assertEqual(body["captureIdentity"]["ctrlPort"], 18072, "health 自报实际监听端口")
        with self.assertRaises(urllib.error.URLError):
            _http_json("GET", "http://127.0.0.1:8072/health")
        reg = A._ctrl_registry()
...
class TestR10CaptureIdentity(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ.pop("SPEEDEX_HK_CTRL_PORT", None)
        self.assertNotIn("mitmproxy", sys.modules, "离线环境无 mitmproxy——env/不可观测路径")

    def _fake_mitmproxy(self, allow_hosts):
        """注入假 mitmproxy 模块（ctx.options.allow_hosts）——离线验证真实生效值路径。"""
        mod = types.ModuleType("mitmproxy")
        mod.ctx = types.SimpleNamespace(options=types.SimpleNamespace(allow_hosts=allow_hosts))
    ...
    def test_unavailable_when_unobservable(self):
        ...
        self.assertEqual(ci, {"allowHosts": None, "allowHostsSource": "unavailable", "ctrlPort": 8072},
    ...
    def test_mitmproxy_options_preferred_over_env(self):
        ...
        self.assertEqual(A._capture_identity()["allowHosts"], [], "空 allow_hosts 如实报告（全量拦截事实）")
    ...
    def test_identity_in_timeline_and_health(self):
        ...
        self.assertEqual(tl["captureIdentity"]["allowHosts"], [self.PAT], "timeline 自带采集身份（每批持久化）")
        self.assertEqual(tl["captureIdentity"]["allowHostsSource"], "env")
        code, body = _http_json("GET", "http://127.0.0.1:8072/health")
        self.assertEqual(code, 200)
        self.assertEqual(body["captureIdentity"], tl["captureIdentity"], "/health 与 timeline 同源")
...
class ChainHeadTests(unittest.TestCase):
    def test_pre_anchor_stale_null_and_closed_window(self):
        ...
        self.assertEqual(c.snapshot("head-window", 1100)["solana"]["status"], "no-pre-anchor-head")
        c.add("solana", 100, "ws-head")
        now[0] = 1200
        c.add("solana", 101, "ws-head")
        self.assertEqual(c.snapshot("head-window", 1100)["solana"]["height"], 100)
        self.assertEqual(c.snapshot("head-window", 4000)["solana"]["status"], "stale-head")
    ...
    def test_hook_freezes_same_window_head_without_network(self):
        ...
        try:
            ...
            with store.lock:
                leg = store._new_leg_locked("head-window", "okx", "robinhood", {"t": 1100, "tw": "synthetic"})
            ...
            self.assertEqual(captured["height"], 100)
            self.assertEqual(store.timeline("head-window")["legs"][0]["chainHead"]["height"], 100)
        finally:
            c.close()
            A.HEADS = original
...
class TestChainHeadSolForkContinuity(unittest.TestCase):
    ...
    def test_sol_parent_continuity_and_fork_reanchor(self):
        ...
        try:
            ...
            self.assertEqual((snap["height"], snap["parent"]), (101, 100), "parent 槽位随样本保留")
            ...
            self.assertEqual(c.snapshot("sol-heads", 1150)["solana"]["status"], "no-pre-anchor-head")
            ...
            self.assertEqual(c.snapshot("sol-heads", 1250)["solana"]["status"], "no-pre-anchor-head")
            ...
            self.assertEqual(c.snapshot("sol-heads", 1350)["solana"]["status"], "no-pre-anchor-head")
            snap = c.snapshot("sol-heads", 1450)["solana"]
            self.assertEqual((snap["status"], snap["height"], snap["parent"]), ("ok", 105, 102))
            # 高度回退（更低 slot）同样清空（与 EVM 同律）
            now[0] = 1500
            c.add("solana", {"parent": 101, "slot": 102, "root": 101}, "ws-head")
            self.assertEqual(c.snapshot("sol-heads", 1450)["solana"]["status"], "no-pre-anchor-head")
    ...
    def test_sol_poll_without_parent_interleaves_without_false_conflict(self):
        ...
        try:
            ...
            c.add("solana", {"parent": 101, "slot": 102, "root": 101}, "ws-head")
            snap = c.snapshot("sol-heads", 1150)["solana"]
            self.assertEqual((snap["status"], snap["height"]), ("ok", 102), "轮询头可被后续 ws 头正向衔接")
...
class TestChainHeadControlPlaneWiring(unittest.TestCase):
    ...
    def test_mark_starts_window_close_stops_and_remark_replaces(self):
        ...
        manifest = {"version": 1, "runId": "run-hw1", "rounds": 1,
                    "legs": [{"platform": "gmgn", "chain": "solana", "rounds": 1}]}
        code, body = _http_json("POST", "http://127.0.0.1:8072/mark", {"runId": "run-hw1", "manifest": manifest})
        self.assertEqual((code, body["ok"]), (200, True))
        self.assertEqual(A.HEADS.run_id, "run-hw1", "/mark 开窗即启动链头采集")
        self.assertEqual(sorted(A.HEADS.samples), ["solana"], "链集合按 legs 明细（非全链默认）")
        # 确定性喂头（不经网络）：开窗后的快照出值
        ...
        code, _ = _http_json("POST", "http://127.0.0.1:8072/mark/close", {"runId": "other"})
        self.assertEqual(code, 200)
        self.assertFalse(A.HEADS.stop.is_set(), "runId 不符不关采集窗")
        # matching runId close → 采集窗关停，快照不再出值
        code, _ = _http_json("POST", "http://127.0.0.1:8072/mark/close", {"runId": "run-hw1"})
        self.assertEqual(code, 200)
        self.assertTrue(A.HEADS.stop.is_set())
        self.assertEqual(A.HEADS.snapshot("run-hw1", A.HEADS.clock() + 100), {}, "关窗后快照为空")
        # re-mark → 新窗显式顶掉旧窗；旧窗迟到的头与旧 runId 快照一律拒绝
        old = A.HEADS
        code, _ = _http_json("POST", "http://127.0.0.1:8072/mark", {"runId": "run-hw2", "manifest": manifest})
        self.assertEqual(code, 200)
        self.assertIsNot(A.HEADS, old)
        self.assertEqual(A.HEADS.run_id, "run-hw2")
        self.assertIsNotNone(A.STORE.marks["run-hw1"]["closed"], "新窗显式关闭旧窗（单窗纪律）")
        old.add("solana", {"parent": 100, "slot": 101, "root": 100}, "ws-head")
        self.assertEqual(len(old.samples["solana"]), 1, "旧窗已关停——迟到头丢弃，既有样本不删")
        self.assertEqual(old.snapshot("run-hw1", old.clock() + 100), {}, "旧窗快照拒绝")
        self.assertEqual(A.HEADS.snapshot("run-hw1", A.HEADS.clock() + 100), {}, "新窗不认旧 runId")
...
class TestKeepAliveJsonRpc(unittest.TestCase):
    ...
    def test_reuses_connection_and_preserves_ua(self):
        class FakeResp:
            status = 200
            def read(self):
                return json.dumps({"result": {"number": "0x64", "hash": "0x" + "aa" * 32}}).encode()
        class FakeConn:
            def __init__(self):
                self.calls = 0
                self.ua = None
        ...
        self.assertEqual(fake.ua, "speedex-hk-timing/1.0", "UA 与 _jsonrpc 一致（robinhood 403 规避）")
    ...
    def test_error_reconnects_and_non200_raises(self):
        ...
        built = []
        ka._connect = lambda: built.append(FlakyConn(True)) or setattr(ka, "_conn", built[-1])
        ok_resp = json.dumps({"result": {"number": "0x65", "hash": "0x" + "bb" * 32}}).encode()
    ...
    def test_injected_rpc_call_respected_and_diag_counts(self):
        ...
        try:
            for _ in range(5):
                sent = c.clock()
                try:
                    result = c._poll_call("robinhood", "eth_getBlockByNumber", ["latest", False])
                    ...
                    with c.lock:
                        ...
                        c.diag["lastErr"]["robinhood"] = f"{type(e).__name__}: {str(e)[:80]}"
                now[0] += 600
            self.assertEqual(len(c.samples["robinhood"]), 4, "第 3 次失败被跳过、其余入缓冲")
...
class TestChainHeadFreshnessAtAnchors(unittest.TestCase):
        ...
        try:
            ...
            for _ in range(100):
                ...
                while next_anchor < len(anchors) and now[0] >= anchors[next_anchor]:
                    ...
                    self.assertEqual(snap["status"], "ok", f"anchor {anchor}: 快照必须新鲜")
    ...
    def test_stale_and_missing_semantics_preserved(self):
        ...
        try:
            ...
            self.assertEqual(snap["status"], "stale-head", "超龄样本如实 stale-head（不谎报新鲜）")
            self.assertEqual(c.snapshot("stale-keep", 4001)["robinhood"]["sampleAgeMs"], 3001)
            c2 = A.ChainHeadWindow(clock=lambda: 1000)
            c2.run_id = "stale-keep"
            c2.samples = {"robinhood": deque(maxlen=512)}
            self.assertEqual(c2.snapshot("stale-keep", 1300)["robinhood"]["status"], "no-pre-anchor-head")
            c2.close()
        finally:
            c.close()
